...
from pipeline.face_matching.mtcnn.exceptions import InvalidImage
from pipeline.face_matching.mtcnn.network.factory import NetworkFactory
...
class MTCNN(object):
    ...
    def __init__(self, p_net: str = None, r_net: str = None, o_net: str = None, min_face_size: int = 20, steps_threshold: list = None,
        ...
        self._pnet, self._rnet, self._onet = p_net, r_net, o_net
    ...
    def __stage1(self, image, scales: list, stage_status: StageStatus):
        ...
        for scale in scales:
            ...
            # print(img_y.shape)
            img_y = tf.make_tensor_proto(img_y, dtype=tf.float32)

            out = get_grpc_predict(self._pnet, 'input_1', img_y)
            # out = self._pnet.predict(img_y)

            out0 = tf.make_ndarray(out.outputs['conv2d_4'])
            out1 = tf.make_ndarray(out.outputs['softmax'])
    ...
    def __stage2(self, img, total_boxes, stage_status: StageStatus):
        ...
        tempimg1 = tf.make_tensor_proto(tempimg1, dtype=tf.float32)

        out = get_grpc_predict(self._rnet, 'input_2', tempimg1)
        # out = self._pnet.predict(img_y)

        out0 = tf.make_ndarray(out.outputs['dense_2'])
        out1 = tf.make_ndarray(out.outputs['softmax_1'])
    ...
    def __stage3(self, img, total_boxes, stage_status: StageStatus):
        ...
        out = get_grpc_predict(self._onet, 'input_3', tempimg1)
        # out = self._pnet.predict(img_y)

        softmax_2 = tf.make_ndarray(out.outputs['softmax_2'])
        dense_5 = tf.make_ndarray(out.outputs['dense_5'])
        dense_6 = tf.make_ndarray(out.outputs['dense_6'])

        # out = self._rnet.predict(tempimg1)

        out0 = np.transpose(dense_5)
        out1 = np.transpose(dense_6)
        out2 = np.transpose(softmax_2)
